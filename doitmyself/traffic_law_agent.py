"""도로교통법 상담 Agent 핵심 로직 (서버 재사용용 모듈).

`traffic_law_z.ipynb`에서 만든 하이브리드 RAG(벡터 + BM25 + RRF) + LangGraph 상담
워크플로를 그대로 옮긴 모듈입니다. 노트북은 대화형 실험에 쓰고, 이 모듈은 FastAPI
서버(`traffic_law_server.py`)에서 import해 재사용합니다.
...
logger = logging.getLogger("traffic_law_agent")
...
# ── 문서 로딩 & 청킹 (traffic_law_z.ipynb와 동일 로직) ───────────────────────
...
logger.info(
    "법령 벡터 %d개 / 판례 벡터 %d개 준비 완료",
    law_store._collection.count(),
    case_store._collection.count(),
)


# ── LangGraph 상담 워크플로 (traffic_law_z.ipynb와 동일 로직) ────────────────
...
logger.info("Agent graph ready (chat=%s, embedding=%s)", CHAT_MODEL, EMBEDDING_MODEL)
